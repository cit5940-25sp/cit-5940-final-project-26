...
//    }
    ...
    public BoardSpace[][] futureBoard(BoardSpace[][] board, BoardSpace destination,
                                      Map<BoardSpace, List<BoardSpace>> availableMoves, Player pc) {
        BoardSpace[][] copyBoard = getCopyBoard(board);
        List<BoardSpace> theOrigins = availableMoves.get(destination);
        if (theOrigins == null || theOrigins.isEmpty()) {
            System.out.println("Origins is null");
        ...
        for (BoardSpace eachOrigin : theOrigins) {
            ...
            while (true) {
                ...
                // (NEW) Only flip if this position is the opponent's color
                BoardSpace.SpaceType opponentColor =
                        (pc.getColor() == BoardSpace.SpaceType.BLACK
                                ? BoardSpace.SpaceType.WHITE
                                : BoardSpace.SpaceType.BLACK);

                if (thePosition.getType() == opponentColor) {
                    thePosition.setType(pc.getColor()); // flip the opponent disc
                }

                // If already reached the destination, break
                if (newRow == destination.getX() && newColumn == destination.getY()) {
                    break;
                }
