...
public abstract class Player {
    ...
    public Map<BoardSpace, List<BoardSpace>> getAvailableMoves(BoardSpace[][] board) {
        ...
        // If it doesn't exist, return the empty map
        ...
        for (int row = 0; row < board.length; row++) {
            for (int column = 0; column < board[row].length; column++) {
                ...
                if (board[row][column].getType() == BoardSpace.SpaceType.EMPTY) {
                    List<BoardSpace> theOrigins = new ArrayList<>();
                    for (int[] eachDirection : availableDirections) {
                        int rowChange = eachDirection[0];
                        int columnChange = eachDirection[1];
                        ...
                        while (checkBounds(newRow, newColumn, board.length) &&
                                board[newRow][newColumn].getType() == opponent) {
                            // Add the opponent's positions to the list
                            opponentPositions.add(board[newRow][newColumn]);
                        ...
                        if (!opponentPositions.isEmpty() && checkBounds(newRow, newColumn, board.length) &&
                                board[newRow][newColumn].getType() == this.getColor()) {
                            // Add the origin position to the list
                            theOrigins.add(board[newRow][newColumn]);
                        }
                    }
                    ...
                    if (!theOrigins.isEmpty()) {
                        allAvailableMoves.put(board[row][column], theOrigins);
                    }
                }
            }
        }
